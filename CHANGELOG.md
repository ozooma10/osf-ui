...
- The game's controlmap overlay (`ControlMap.txt` / `ControlMap_Custom.txt`) is consumed in its own scan-code space with no layout-dependent translation, so the "Starfield (…)" rows are correct on every layout; the curated `vanillakeys.json` rows now genuinely match the engine's positional defaults on non-QWERTY layouts. Plugin and host must update together (pipe protocol 6).
