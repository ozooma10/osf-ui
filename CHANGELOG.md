...
- The in-world surface could take over the game's own rendering instead of just its screen, leaving most of the world invisible. It identified its target purely by texture size, so a size the game also used internally would capture the wrong thing entirely. It now checks that the target really is an ordinary material texture and not something the renderer owns, and its placeholder uses a shape the renderer would never allocate.
- The in-world surface no longer replaces a vanilla texture to find its screen. It ships its own placeholder texture under `Textures/OSFUI/` and points the two cockpit avionics materials at it, so the game's own screen texture is left untouched and nothing else can accidentally match the surface's target. Both the texture and the material overrides are now generated by checked-in tools rather than being hand-made files. Ship-manufacturer avionics screens (Deimos, HopeTech, Stroud, Taiyo) use their own textures and are unaffected either way. The surface remains experimental, rendering-only, and off unless `worldSurfaceView` is set.
