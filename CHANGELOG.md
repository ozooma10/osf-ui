...
- **The native ABI moves to major 2, which is a hard break.** A plugin built against 1.x gets `nullptr` from `OSFUI_RequestBridge` and raises **compat.legacy-api** naming its DLL, so the failure is visible rather than a mod that quietly does nothing. Recompiling against the shipped `sdk/OSFUI_API.h` is the whole migration for most plugins; commands are no longer auto-acked and no longer receive an injected `requestId`, so a handler that needs to answer must register as a request instead.
...
- Native ABI **2.0**. `SetViewState(modId, key, payloadJson)` is new and closes the largest gap in the 1.x surface: state was Papyrus-only, so a native plugin had to invent its own reload handshake (listen for a view-defined hello, re-push) and every one of those was a chance to get the blank-after-reload bug. Set the value once and the runtime replays it to every document of that mod, forever. Native state is not session-scoped — wiping a HUD's configuration on every save load would be a bug — while Papyrus `SetView*` state still is, because it can hold form identities.
- A registered *command* is now unambiguously a send endpoint: nothing is auto-acked, no `requestId` is injected into its payload, and an `osfui.request()` naming it is refused with `wrong-endpoint-kind` rather than hanging. Use `RegisterRequest` when the caller needs an answer; the deferred `Request::Respond` / `Request::Reject` token from 1.7 is unchanged.
