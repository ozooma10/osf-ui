...
- `create-osfui` now generates all Papyrus and native menu/HUD projects against the 2.0 authoring surface: manifests and settings declare their target version, retained state uses qualified keys and `SetViewState`, and browser mocks use the current envelope and endpoint-handler contracts. Fresh projects therefore pass `npm run check`, retain HUD data across page recreation, and no longer wait on dead subscriptions or mock requests that never settle.
