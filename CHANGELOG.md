...
- `osfui check`, `build`, and `package` now validate generated manifests and drop-in settings JSON against the packaged 2.0 schemas, including malformed JSON and a settings id that disagrees with its filename. The toolchain can also preview, build, and package a pre-2.0 project through the frozen 1.x façade. Malformed targets are still refused, and new scaffolds and published typings remain 2.0-only.
...
- The `@osfui/cli` authoring harness mirrors the strict 2.0 API for current projects. An explicitly pre-2.0 target selects the same frozen 1.x façade as the game; newly scaffolded projects still use only 2.0.
