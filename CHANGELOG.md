...
- Added the native first slice for an opt-in in-world Web UI surface: a configured view runs in its own WebView2 host and shared-texture ring, remains alive while the fullscreen overlay is closed, and follows completed ring slots. No world material assets ship yet: the loose cockpit-material experiment proved unsafe and was removed from deployment, so this remains developer infrastructure until a Creation Kit-authored screen provides a stable target. Interaction and multiple instances also remain follow-up work.
...
- Stopped packaging the experimental loose cockpit materials and placeholder texture after they made cockpit/world geometry disappear even with the runtime surface disabled. Retargeting filename fields while retaining Bethesda's embedded material resource identities is not a safe asset-authoring technique. The native binding still rejects renderer-owned targets, but world-surface assets now require a properly authored custom material instead of overriding vanilla avionics materials.
- A malformed experimental placeholder DDS could also take world rendering down rather than merely showing a missing texture. Its generator now emits the pitch/mip header shape accepted by Starfield, while the file remains an investigation artifact and is no longer packaged.
