...
- Mod ids are now opaque names instead of mandatory lowercase `<author>.<modname>` pairs. Dotless names, spaces, mixed case, underscores, and multiple dots work across views, settings, localization, Papyrus, and the native API; only the filesystem/URL path-safety boundary and 64-byte cap remain. `osfui` is explicitly reserved case-insensitively for the framework, plugin endpoints no longer infer ownership by counting dots, and System Health carries explicit mod/platform ownership so a dotless mod cannot be mistaken for an OSF UI subsystem.
- OSF UI now uses one documented vocabulary across the OSF UI runtime, SDK, authoring guides, and built-in views: a stable view is distinct from each browser document instance; the OSF UI runtime, browser host, web renderer, compositor, and mod backends have unambiguous roles; and game input actions, bindings, mod hotkeys, engine input contexts, and hotkey contexts are named separately. Current-facing docs no longer teach the retired 1.x bridge envelope, a nonexistent manifest `id`, or a menu stack. Remaining compatibility spellings are unchanged. The built-in views are now consistently called **Mod Settings** and **Keybindings**, including the default **MOD SETTINGS** pause-menu entry, and Mod Settings labels authored views as **Menus** and **HUDs** instead of mixing systems, terminals, and overlays.
- Existing integrations keep working without a scheduled compatibility cutoff. Views declaring a 1.x `targetVersion` continue through the frozen 1.x helper and the six older Papyrus natives remain bound, while current projects receive the strict 2.0 web API. The native bridge returns to one append-only ABI 1.x line: old binaries use their unchanged vtable prefix, retained state remains the ABI 1.8 tail, and strict one-way sends are appended in ABI 1.9. System Health no longer treats a working older consumer as a problem; genuinely different ABI majors are still refused and reported.
- The **Debug mode** toggle is gone from Mod Settings; third-party `debugOnly` views now follow the `devMode` flag in `config.json`, the same switch that already governs verbose logging, hot reload and F12 DevTools.
...
- Menu opens are refused when the overlay cannot actually draw, not just when its install-time hooks failed. The draw path's command-list hooks are taken lazily on the first frame and can fail then, which previously still admitted an invisible overlay that captured input.
...
- Removed the repository's JavaScript/TypeScript view-authoring toolchain to reset that product around new goals. The `@osfui/cli` package, `create-osfui` scaffolder, browser harness, generated starter templates, and their frontend mock integration are no longer shipped or maintained; OSF UI's runtime web API, schemas, SDK declarations, shared helper, and built-in frontend build remain available.
...
- Removed the developer-only Web Performance Lab and the live render-stat overlay, along with their per-frame counters, sampling IPC, and injected page UI.
- System Health remains the local in-game status surface for current errors, warnings, resolved history, system facts, and safe recovery actions. Removed the separate bug-report/crash-report workflow instead: there is no whole-report export, log upload or submission, post-crash prompt, log-folder shell action, or external issue/mod-page opener.

### For view authors

- The 2.0 shared kit drops exports that no shipped view used: `--osf-bg-canvas`, `--osf-inset-lit`, `--osf-shadow-sm`, `--osf-space-1`, `--osf-space-5`, `--osf-space-9`, `--osf-steel-100`, `--osf-surface-input`, `--osf-surface-panel`, `--osf-text-2xl`, `--osf-text-4xl`, `--osf-text-lg`, `--osf-text-xl`, `--osf-tracking-mega`, `--osf-tracking-wide`, `--osf-void-400`, `.osf-section-head`, `.osf-tag`, and `.osf-tricolor`. The direct `.osf-note--info` and `.osf-note--danger` classes are also gone; schema note tones keep their existing appearance through the settings renderer.
...
- Views can now call any GLOBAL function on any Papyrus script with `osfui.papyrus.call(script, function, ...args)`. Scalar arguments retain their string, integer, float, or boolean types, with `osfui.papyrus.float(value)` available when a whole-valued number must remain a float. Papyrus mods can therefore expose a loose PEX without requiring an ESM, startup quest, load-game registration, or Spriggit.

### Breaking changes

- **The 2.0 shared bridge helper API drops the 1.x aliases for 2.0 views.** `emit`, `call`, `action`, `data.push`, `data.state` and the top-level `t`/`localize`/`locale`/`applyAccent` are absent from a view targeting 2.0. A manifest explicitly targeting pre-2.0 selects an isolated frozen façade that restores the final documented 1.x API.
...
- **Papyrus supersedes `PushToView`, `PushFormsToView`, and the four `RegisterForViewActions*` registrations without removing them.** The frozen calls keep their published behavior. New scripts should publish with `SetView*` (replayed to every fresh document), announce with `SendViewEvent` (never replayed), and use `ListenForViewActions{,Static}`.
- **Reads that secretly subscribed are gone.** `settings.get`, `views.get`, and `i18n.get` were requests with an invisible side effect — asking once enrolled you in every future push. They are now plain state keys (`osfui/settings`, `osfui/views`, `osfui/i18n`): subscribing replays the current value immediately and again on every change and every reload, so there is nothing to request and nothing to re-request.
...
- **Deleted from the strict 2.0 path.** `hud.show`/`hud.hide` (aliases of `menu.open`/`menu.close`), `osfui.textFocus`, `ui.action`, and `ui.papyrusRequest` are absent from a 2.0 view, as are the `runtime.ready`, `ui.result`, `ui.error`, `settings.ack`, `settings.data`, `views.data`, `i18n.data`, `diagnostics.data`, `data.push`, `data.state`, `papyrus.result`, `runtime.pong`, and `game.data` message types. Routing metadata now travels beside the payload rather than inside it, so a payload field can no longer overwrite the command name. The frozen façade reconstructs the public 1.x aliases and shapes only for an explicitly pre-2.0 view.
...
- **Breaking (configVersion 2):** `config.json` no longer takes `views` or `warmViews`; there is no config-v1 compatibility branch, so removed fields are ordinary unknown keys and are ignored with the normal warning. Every valid installed view is discovered automatically (in deterministic id order) and instantiated on first use. `view` remains the default menu for the toggle key. Saved setting-key migrations and unknown setting-value preservation remain intact.
- **The first-load handoff is removed.** A newly instantiated menu remains closed, without taking input or pausing the game, until its main-frame load succeeds; the existing fresh-frame gate still prevents stale cached pixels when it opens. Failed loads stay closed while automatic recovery handles them and records failures in `OSF UI.log` instead of maintaining a separate loading/retry view. The handoff's manifest `accent` and `readySignal` fields, `view.ready` send, `markReady()` / legacy `viewReady()`, `osfui/handoff` state, and `osfui.handoffRetry` send are removed; settings-schema accents and `osfui.theme.applyAccent()` remain unchanged.
...
- Views, including Mod Settings, are instantiated only on first use (or effective HUD auto-start). Once created, an ordinary close hides the document but keeps it resident for the rest of the Starfield process; there is no idle suspension, TTL, or hidden-view cap. This removes the asynchronous cross-process lifecycle at the cost of memory growing with the number of distinct views opened during a session. Browser-host recovery still recreates every instantiated view.
...
- The optional `OSFUI_JSON.h` conveniences (`JsonClient` and the typed `Respond` overloads) now serialize payloads containing invalid UTF-8 by substituting the Unicode replacement character instead of failing the call or rejecting the request with `serialization-error`, matching how the OSF UI runtime itself serializes throughout.
- The native bridge is additive ABI **1.9**. Every ABI 1.x binary receives the same bridge and keeps its published vtable prefix and behavior; `SetViewState` remains the ABI 1.8 tail, while strict `RegisterSend` / `UnregisterSend` append at 1.9. Frozen `RegisterCommand` still provides request-ID injection and auto-ack for old consumers. Unrelated majors remain refused.
- `SetViewState(modId, key, payloadJson)` gives native plugins retained state that OSF UI replays to every document of that mod. Use ABI 1.9 `RegisterSend` for one-way messages and ABI 1.7 `RegisterRequest` plus `Request::Respond` / `Reject` for work requiring an outcome. The optional JSON facade exposes `JsonRequest::Name()` for that request endpoint while retaining `Command()` as a source-compatible alias for the frozen ABI field.
- `RegisterView` now validates ordinary plugin-shipped views without eagerly instantiating them; an explicit `RegisterView` still honors manifest `openOnStart` immediately (menus included — that path is plugin opt-in, unlike discovery, which never auto-starts menus). `SendToWeb` keeps a bounded FIFO holdback for known discovered-but-uninstantiated targets, so sending an initial event immediately before opening a view retains the existing first-paint ordering guarantee. The per-view 64-message drop-oldest bound now applies to every target — including instantiated views, which were previously unbounded between ticks — matching the renderer's own per-view queue bound; on overflow, the newest happenings are retained for delivery.
...
- Browser-host IPC now uses verified peers, heartbeat liveness, presentation epochs, and physical scan-code identity. The OSF UI runtime and browser-host binaries must be updated together.
- A deployable Papyrus-only example now demonstrates `onPress` with no `.esp`, startup quest, or callback registration, including rebinding, save-load, suppression, and failure-log checks.
...
- The `osfui/views` state key (which replaces `views.data`) carries `autoStart` and `autoStartMutable`, and the platform-private `osfui.setViewAutoStart` request (Mod Settings only) persists a HUD's next-launch choice. Manifest `openOnStart` is now documented as the HUD author default; for discovered menus it is ignored.
- An ordinary close preserves a view's document state for the rest of the session. F5, developer reload, document crash recovery, or browser-host recovery can still create a fresh document, so use state — `SetView*` from Papyrus, `SetViewState` from a plugin — for data that must survive recreation. One-shot events are never replayed to a later document: Papyrus events with no instantiated target are dropped, while native `SendToWeb` has only a bounded pre-instantiation holdback for open ordering.
