...
- Removed the diagnostic-reporting stack entirely: the System Health interface, copied reports, reporting-oriented shell actions, retained diagnostics state, runtime health collection, and native ABI 2.0 issue-publication methods are gone. Failures still write to the existing logs, automatic view recovery and typed request errors are unchanged, and the frozen ABI 1.x reporting slots remain only as fail-closed no-ops so old DLL vtables stay safe.
...
- ABI 1.0–1.8 binaries temporarily receive a frozen ABI 1.8 adapter instead of `nullptr`. It preserves settings, hotkeys, view registration, retained state, registered requests, and the old `RegisterCommand` request-ID/auto-ack path while warning once per DLL that support ends in 2.1.0. Its historical reporting slots now return `false`. ABI 2.x remains strict and unrelated majors are still refused.
