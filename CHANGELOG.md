...
- **The 2.0 helper surface drops the 1.x aliases.** `emit`, `call`, `action`, `viewReady`, `data.push`, `data.state` and the top-level `t`/`localize`/`locale`/`applyAccent` are gone rather than deprecated: each existed because 1.x never settled whether a caller wanted a completion or a value, and keeping them alive would have meant re-implementing inside 2.0 the exact bookkeeping 2.0 exists to delete. A manifest explicitly targeting a pre-2.0 version is refused on open and reported through System Health; the runtime and authoring harness no longer carry a `legacyApi` mode or compatibility façade.
- **The native ABI is now 2.0.** `CommandFn`, `RegisterCommand`, and `UnregisterCommand` are replaced by strict `SendFn`, `RegisterSend`, and `UnregisterSend`; `RegisterRequest` remains the reply-bearing path. Send payloads are passed verbatim, and a request naming a send is refused without injected `requestId` data or an automatic acknowledgement. ABI 1.x callers receive `nullptr` plus a bounded local System Health error naming the outdated DLL.
...
- **Breaking (configVersion 2):** `config.json` no longer takes `views` or `warmViews`; there is no config-v1 compatibility branch, so removed fields are ordinary unknown keys and are ignored with the normal warning. Every valid installed view is discovered automatically (in deterministic id order) and created on first use; the handoff and Mods surfaces are pinned warm internally. `view` remains the default menu for the toggle key. Saved setting-key migrations and unknown setting-value preservation remain intact.
...
- Native ABI **2.0** makes `SetViewState(modId, key, payloadJson)` part of the baseline and replaces ambiguous commands with strict sends. Set retained state once and the runtime replays it to every document of that mod; use `RegisterSend` for one-way messages and `RegisterRequest` plus `Request::Respond` / `Reject` for work requiring an outcome.
...
- The `@osfui/cli` authoring harness now mirrors the strict 2.0 API end to end: runtime metadata, injected events, public mock typings, translation wrappers, and newly scaffolded projects all use the current envelopes and callback contracts. A pre-2.0 `targetVersion` no longer selects a separate helper mode.
