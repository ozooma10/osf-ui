...
namespace OSFUI::API
{
	...
	struct IOSFUIBridge
	{
		...
		// --- view-open preflight (ABI 1.11). Appended; every older slot is frozen. ---
		// Register one handler for an exact qualified view id. Registration is
	...
	public:
		...
		// --- 1.11 native view-open preflight ---
		bool RegisterViewOpenPreflight(const char* a_viewId, ViewOpenPreflightFn a_handler, void* a_user) const noexcept
		{
			return Has(Feature::kViewOpenPreflight) && _bridge->RegisterViewOpenPreflight(a_viewId, a_handler, a_user);
		}
		void UnregisterViewOpenPreflight(const char* a_viewId) const noexcept
		{
