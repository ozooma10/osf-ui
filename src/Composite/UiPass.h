...
{
	// Hooks into Starfield's native Scaleform UI render pass to draw under it.
	//
	...
	// input, not the UI layer. The hand-off shape identifies that graph and
	...
	//
	// Installs the Begin/End/Composite hooks and enables the UI-pass draw. This
	// is the only path that puts the overlay on screen — the present-time
	// renderer was retired — so a false return means OSF UI cannot draw this
	// session.
	bool Install();

	// Whether the UI pass can actually draw RIGHT NOW. Install()'s return value
	// is only the vtable-hook half; the command-list hooks are taken lazily on
	// the first frame from a render worker, and their self-test can fail (or a
	// later fault can disable drawing) long after Install() said yes. Callers
