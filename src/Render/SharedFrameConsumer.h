...
namespace OSFUI
{
	...
	public:
		...
		void SetPresentation(std::uint64_t a_epoch, bool a_accept)
		{
			...
			m_state.SetPresentation(a_epoch, a_accept);
		}
		...
		{
			std::scoped_lock lock(m_mutex);
			return m_state.Publish(a_slot, a_serial, a_width, a_height, a_epoch);
		}
		...
		bool HasRecorded() const { return m_hasRecorded.load(std::memory_order_acquire); }
		bool HasRecorded(std::span<const std::uintptr_t> a_lists) const
		{
			std::scoped_lock lock(m_mutex);
			return m_state.HasRecorded(a_lists);
		}
		void Submitted(std::span<const std::uintptr_t> a_lists, std::uintptr_t a_queue, std::uint64_t a_value)
		{
			std::scoped_lock lock(m_mutex);
			m_state.Submitted(a_lists, a_queue, a_value);
			m_hasRecorded.store(m_state.HasRecorded(), std::memory_order_release);
		}
		void Completed(std::uintptr_t a_queue, std::uint64_t a_value)
		{
			std::scoped_lock lock(m_mutex);
			m_state.Completed(a_queue, a_value);
		}
		bool HasReads() const
		{
			std::scoped_lock lock(m_mutex);
			return m_state.HasReads();
		}
		SharedFrameState::Releases TakeReleases()
		{
			std::scoped_lock lock(m_mutex);
			return m_state.TakeReleases();
		}

	private:
		mutable std::mutex m_mutex;
		SharedFrameState m_state;
		std::atomic_bool m_hasRecorded{ false }; // queue-hook fast path; state owns the truth
		std::optional<SharedRingDesc> m_pendingRing;
