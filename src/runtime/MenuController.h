...
namespace OSFUI
{
	// Registered surfaces (menus + HUDs) and their open state; derives the desired global UI policy from the top-of-stack surface.
	...
	public:
		...
		// Remove a surface entirely (closing it first if open). Returns true if the
		// open-state changed (caller must re-apply policy). Called when a view is torn
		// down at runtime (crash-recovery exhaustion) so nothing can reopen a surface
