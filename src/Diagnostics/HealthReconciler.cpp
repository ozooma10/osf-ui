...
namespace OSFUI
{
	void HealthReconciler::ReportViewLoad(HealthRegistry& a_healthRegistry, std::string_view a_viewId, bool a_failed, std::string_view a_description, int a_errorCode, std::uint32_t a_attemptsLeft)
	{
		const std::string id(a_viewId);
		const auto retrying = "view.load-retrying:" + id;
		const auto failed = "view.load-failed:" + id;
		if (!a_failed) {
			a_healthRegistry.Resolve(retrying);
			a_healthRegistry.Resolve(failed);
			return;
		}
	...
	void HealthReconciler::ReportRendererHealth(HealthRegistry& a_healthRegistry, std::string_view a_code, bool a_active, std::string_view a_detail, bool a_rendererAvailable)
	{
		if (a_code.empty()) return;
		if (!a_active) {
			a_healthRegistry.Resolve(a_code);
			return;
		}
	...
	void HealthReconciler::ReportProtocolMisuse(HealthRegistry& a_healthRegistry, std::string_view a_viewId, std::string_view a_code, std::uint32_t a_count)
	{
