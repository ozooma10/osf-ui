...
//
// `n: null` marks a DEAD cell — drawn, but not bindable by mods, for one of
// two reasons:
//   * the name does not resolve natively (the punctuation keys: - = [ ] \ ; ' , . /), or
//   * Esc, which IS resolvable but is reserved: the capture flow treats a
//     press of it as "cancel", so binding it would make rebinds unescapable.
