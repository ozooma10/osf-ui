...
Function Refresh() Global
    OSFUI.SetState("__OSFUI_MOD_ID__", "clicks", 0)
EndFunction
...
Function Bump(int total) Global
    OSFUI.SetState("__OSFUI_MOD_ID__", "clicks", total)
    Var[] noticeArgs = new Var[1]
    noticeArgs[0] = "JavaScript called a GLOBAL Papyrus function"
    OSFUI.EmitEvent("__OSFUI_MOD_ID__", "notice", noticeArgs)
EndFunction

; Next steps:
;   - Real forms: OSFUI.SetState accepts one Form through Var, and SetStateForms publishes a Form array. Runtime FormIDs are session-scoped, so never store a serialized identity across a save.
