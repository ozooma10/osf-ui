...
Function OnHotkey(string asModId, string asKey) Global
    ...
    int strength = OSFUI_Settings.GetInt(asModId, "strength", 50)
    string mode = OSFUI_Settings.GetString(asModId, "mode", "normal")
...
;     OSFUI_Settings.SetBool/SetInt/SetFloat/SetString write back through the same validation the menu uses.
