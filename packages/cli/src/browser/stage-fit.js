...
// Two staged modes model that:
//
//   'fixed' — the literal declared box, letterboxed in the available pane.
...
//
// Neither staged mode caps the scale at 1:1 — filling a 1080p pane with a
// 900-row view at 1.2x IS the in-game text size; capping would render
// everything smaller in the harness than in game.

export const STAGE_MODES = ['fixed', 'fill', 'off'];

/** The mode the toolbar button switches to next. */
export function nextStageMode(mode) {
  const index = STAGE_MODES.indexOf(mode);
  return STAGE_MODES[(index + 1) % STAGE_MODES.length] ?? 'fixed';
}

/**
 * Fit a stageW x stageH reference frame into an availW x availH pane.
 * Returns the pre-scale stage size in CSS pixels plus the uniform scale;
 * 'off' is the caller's branch (no frame, no maths).
 */
export function computeFit(availW, availH, stageW, stageH, mode = 'fixed') {
  const paneW = Math.max(1, availW);
  const paneH = Math.max(1, availH);
  const frameW = Math.max(1, stageW);
  const frameH = Math.max(1, stageH);
  if (mode === 'fill') {
    // Height fixes the scale (reference rows, as in game); width is whatever
    // is left over, so the stage covers the pane exactly.
    const scale = paneH / frameH;
    return { scale, width: paneW / scale, height: frameH };
  }
  const scale = Math.min(paneW / frameW, paneH / frameH);
  return { scale, width: frameW, height: frameH };
}
