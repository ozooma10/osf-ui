// Pseudo-locale transform (ported from the repo harness). Dev only.
//
// Accent every letter (glyph coverage), pad ~30% (German-ish expansion), and
// bracket the string. The brackets are the point: unbracketed text on screen
// is a string that never went through the localization path, and anything
 ...
 * so the glyph set stays reviewable: the shipped font must cover exactly
 * these for pseudo mode to be legible.
 */
...
/** Pseudo-localize a flat address->string catalog. */
export function pseudoizeStrings(strings) {
  const out = {};
  for (const [key, value] of Object.entries(strings || {})) out[key] = pseudoize(value);
  return out;
}
