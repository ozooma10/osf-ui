...
namespace osfui::wv2
{
	...
	[[nodiscard]] inline bool EqualsAsciiCaseInsensitive(std::wstring_view a_left, std::wstring_view a_right) noexcept
	{
		...
		{
			if (LowerAscii(a_left[i]) != LowerAscii(a_right[i])) {
				return false;
			}
		}
		return true;
	...
	[[nodiscard]] inline bool IsSafeVirtualPath(std::wstring_view a_path)
	{
		...
		for (const auto ch : a_path) {
			lowered.push_back(LowerAscii(ch));
		}
		if (lowered.find(L"%2f") != std::wstring::npos || lowered.find(L"%5c") != std::wstring::npos)
		{
			return false;
		}
		std::size_t start = 0;
	...
	[[nodiscard]] inline std::optional<HttpsUri> ParseHttpsUri(std::wstring_view a_uri)
	{
		...
		for (const auto ch : authority)
			parsed.host.push_back(LowerAscii(ch));
	...
	{
		return IsHttpsUriForHost(a_uri, a_viewHost);
	}
