...
| `bridge_api_tests.cpp` | `BridgeApi`: ABI 1.9 constants and additive feature gates, frozen `RegisterCommand` plus strict `RegisterSend`/`RegisterRequest` routing, unsupported-major caller observations, local health-operation validation/queueing, retained state, plugin endpoint-shape enforcement, first-wins duplicate refusal, unregister-then-reregister replacement, qualified `RegisterView` ids, discovery-aware `RequestMenu` validation, pre-ready delivery to a first lazy bridge, and the registry-apply/dispatch round trip through a real `MessageBridge`. **Note:** `BridgeApi` is a process singleton, so its sections share state and run in order |
| `v1_native_bridge_tests.cpp` | Append-only ABI 1.x compatibility: exact 1.8 prefix-compatible vtable access through the current 1.9 implementation, frozen command request-ID injection and auto-ack, typed requests, health publication, Suit Protocol-shaped settings/two-hotkey use, and retained state |
| `papyrus_action_tests.cpp` | Papyrus events/state/requests plus menu-native behavior: frozen compatibility and strict fixed-name callback registration, deprecated transient pushes, typed retained state replacement/replay/teardown, one-shot correlated replies/rejection/timeout, bounded queues, open/close results, and `BSFixedString` case folding |
| `papyrus_form_tests.cpp` | Form references across the bridge: frozen `PushFormsToView` and strict retained `SetViewForms` capture FormIDs on the VM thread and serialize on the main thread (identity fields, `FORM_ENUM_STRING` signatures + numeric fallback, null-slot preservation for `None`/deleted forms), plus the `GetFormById`/`GetFormsById` resolvers (decimal + hex parse matrix, quiet stale references, bulk order/length) |
...
| `runtime_lifecycle_contract_tests.cpp` | Source-linked static characterization of the mature cross-component view lifecycle: lazy instantiation, process-lifetime document residency, hidden preparation before bridge delivery and reveal, pending-open cancellation, presentation epochs and stale-frame rejection, fail-closed menu admission, transition-menu CloseAll, reveal timeout cleanup, HUD-preserving menu replacement, and terminal-load-failure teardown. It does not execute Starfield, WebView2, D3D12, or prove host/in-game behavior |
...
| `v1_navigation_tests.cpp` | Frozen pre-2.0 view selector insertion, including existing queries, fragments, and replacement of an authored selector |
