...
int main()
{
...

	{
		...
		auto frame = session.Update(state, Mode::kDefault, 0.0);
		assert(session.Active());
		assert(frame.buttonEdgeCount == 0);  // held-at-open is only a baseline

		state.buttons = kA | kB;
		frame = session.Update(state, Mode::kDefault, 0.1);
		...
		state.buttons = 0;
		frame = session.Update(state, Mode::kDefault, 0.2);
...

	{
		...
		(void)session.Update(state, Mode::kDefault, 0.0);
		state.buttons = kDPadUp | kDPadDown | kDPadLeft | kDPadRight | kA | kB;
		const auto frame = session.Update(state, Mode::kDefault, 0.1);
...

	{
		GamepadSession session;
		State state{ .connected = true };
		(void)session.Update(state, Mode::kRaw, 0.0);
		state.buttons = kDPadUp;
		auto frame = session.Update(state, Mode::kRaw, 0.1);
		assert(frame.buttonEdgeCount == 1);
		assert(frame.buttonEdges[0].action == GamepadSession::Action::kNone);
	}

	{
		...
		frame = session.Update(state, Mode::kDefault, 0.20);
		assert(!frame.axesChanged);
		assert(frame.navigationAction == GamepadSession::Action::kNone);

		frame = session.Update(state, Mode::kDefault, 0.55);
		assert(frame.navigationAction == GamepadSession::Action::kRight);
	}

	{
		...
		auto frame = session.Update(state, Mode::kRaw, 0.0);
		assert(frame.axesChanged);
		assert(frame.navigationAction == GamepadSession::Action::kNone);
		...
		frame = session.Update(state, Mode::kDefault, 0.1);
		assert(!frame.axesChanged);
		assert(frame.wheelDelta == 0);  // switching modes starts a fresh scroll interval
		assert(session.Update(state, Mode::kDefault, 0.2).wheelDelta == 0);
		assert(session.Update(state, Mode::kDefault, 0.3).wheelDelta == 0);
		frame = session.Update(state, Mode::kDefault, 0.4);
		assert(frame.wheelDelta == 120);
	}

	// Scrolling covers the same distance at different update rates, in both directions.
	for (const int updatesPerSecond : { 32, 64, 128 }) {
		for (const float direction : { -1.0f, 1.0f }) {
			GamepadSession session;
			const State state{ .connected = true, .ry = direction };
			assert(session.Update(state, Mode::kDefault, 1000.0).wheelDelta == 0);
			int total = 0;
			for (int i = 1; i <= updatesPerSecond; ++i) {
				total += session.Update(state, Mode::kDefault,
					1000.0 + static_cast<double>(i) / updatesPerSecond).wheelDelta;
			}
			assert(total == static_cast<int>(direction) * 8 * 120);
		}
	}

	{
		GamepadSession session;
		const State state{ .connected = true, .lx = 0.8f, .ry = 1.0f };
		assert(session.Update(state, Mode::kDefault, 1000.0).wheelDelta == 0);
		assert(session.Update(state, Mode::kDefault, 1000.0625).wheelDelta == 0);
		const auto afterStall = session.Update(state, Mode::kDefault, 1010.0);
		assert(afterStall.wheelDelta == 120);  // bounded movement, no ten-second catch-up
		assert(afterStall.navigationAction == GamepadSession::Action::kRight);
		const auto sameTime = session.Update(state, Mode::kDefault, 1010.0);
		assert(sameTime.wheelDelta == 0);
		assert(sameTime.navigationAction == GamepadSession::Action::kNone);

		assert(session.End());
		assert(session.Update(state, Mode::kDefault, 2000.0).wheelDelta == 0);
		assert(session.Update(state, Mode::kDefault, 2000.0625).wheelDelta == 0);
		assert(session.Update(state, Mode::kDefault, 2000.125).wheelDelta == 120);
	}

	{
		...
		frame = session.Update(state, Mode::kDefault, 1.0);
...

	{
		...
		(void)session.Update(state, Mode::kButtons, 0.0);
		state.buttons = kDPadUp;
		auto frame = session.Update(state, Mode::kButtons, 0.1);
		assert(frame.buttonEdges[0].action == GamepadSession::Action::kUp);
		...
		frame = session.Update(state, Mode::kButtons, 0.66);
