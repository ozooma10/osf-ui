...
if has_config("with_webview2") then
    ...
    target("osfui-webview2-host")
        ...
        -- non-ASCII characters in the embedded render-stats overlay JS resources
        -- into mojibake on screen.
        set_encodings("utf-8")
        set_runtimes("MT")
        add_rules("mode.debug", "mode.releasedbg")
        add_files(
            "tools/webview2_host/**.cpp", "tools/webview2_host/**.rc",
            "tools/webview2_shared/**.cpp")
        add_headerfiles("tools/webview2_host/**.h", "tools/webview2_shared/**.h")
        add_extrafiles("tools/webview2_host/scripts/**.js")
