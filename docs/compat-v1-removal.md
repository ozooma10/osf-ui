...
**2.1.0**. The authored 2.0 helper, public 2.0 SDK, and strict 2.0 dispatch are
...
that behavior reach the runtime and authoring tools.
...
## Remove helper composition and navigation selection
...
- `tools/webview2_host/HostApp.cpp` and `HostCommands.inl`: remove the v1
...
- Runtime view discovery/opening must refuse an explicitly pre-2.0
...
- `src/runtime/RuntimeDiagnostics.{h,cpp}` and
  `DiagnosticsReconciler.{h,cpp}`: remove the legacy ABI/Papyrus warning
  producers and compatibility-log ledger. Keep genuinely unsupported ABI-major
  reporting.
- `frontend/src/lib/settings/diagnostics.ts`: remove the three temporary 1.x
  warning cards and restore unsupported pre-2/ABI copy appropriate for 2.1.
...
with the documented 2.1 diagnostic.
