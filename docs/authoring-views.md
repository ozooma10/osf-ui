...
- **`targetVersion`** is `"<major>[.<minor>[.<patch>]]"` and advisory. When the running OSF UI is *older* than the target, a warning goes to `OSF UI.log` and the Mods surface shows a "needs update" badge next to the OSF UI version with your mod named in the tooltip. When the declared target is *older than 2.0*, OSF UI selects the 1.x helper façade over the current transport so the view can keep running unchanged while you migrate. A malformed or undeclared value is treated as current rather than guessing which helper shape the page expects.
