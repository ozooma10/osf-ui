...
| **HDR / 10-bit / multi-format PSO + frame-gen swapchain** | ❌ | `D3D12Compositor.cpp` uses a single-format PSO; a 2nd format is skipped, and under DLSS-G/frame-gen the scanned-out swapchain isn't selected. **D3D12/Starfield-only cost Prisma UI never pays** — budget real time. | L |
