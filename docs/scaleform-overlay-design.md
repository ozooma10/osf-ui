# Scaleform overlay: drawing inside Starfield's UI render pass
...
`R16G16B16A16_FLOAT`; the overlay hook accepts both and records through a matching typed
...
The overlay hook classifies its own frame graph. At the start of each render region it
...
`ScaleformOverlayHook` hooks slot 7 on `ScaleformBegin`, `ScaleformEnd`, and
...
Any other partial or foreign hook disables overlay drawing. Since the present-time
renderer has been retired, menu opens are then refused so an invisible overlay
cannot capture input; the failure is logged at error level by both `ScaleformOverlayHook`
and `Runtime`.
...
the known-good Scaleform-overlay behavior from before `b8e3643`; root-signature and
...
the newest publication is incomplete, the overlay reuses the last ready slot rather
...
- the Scaleform target descriptor supplies output dimensions;
- the Scaleform handoff shape supplies FG classification; and
- Present-hook liveness monitoring is unnecessary because no Present dependency remains.

## Diagnostics

The Scaleform overlay is unconditional; there is no `uiPassDraw` switch and no present-time
fallback to select. A knob whose off position renders no UI is not a useful
compatibility control, and the hook's fail-closed slot check declines every
...
- HDR and `_SRGB` backbuffers no longer suppress the overlay. The Scaleform hook renders
...
- Frame Generation no longer suspends drawing. The Scaleform hook infers FG directly from
...
frame graph, FG target selection, and hand-off decode are baked into the overlay hook.
Only its own load-bearing hooks remain: the `ScaleformBegin`/`End`/
...
- load-safe, hitch-free behavior from the preceding Scaleform-overlay acceptance sessions.
...
removed. Output sizing and FG target selection come from `ScaleformOverlayHook`, while
shared-ring adoption runs from `Submit()` on the tick thread.

The universal Scaleform-overlay build still requires the full in-game compatibility
