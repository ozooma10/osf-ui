...
**6.4 Still open: `osfui:trace` from the dev harness.** Whether the trace flag should also be settable per-view from the harness, mirroring `openDevTools`, shipped **unimplemented**. The flag is `localStorage["osfui:trace"]`, read once at helper load (`frontend/src/shared-kit/osfui.js`); there's no pipe command for it. A deliberate deferral: localStorage already works for every view including world surfaces, and a second control path for a debug toggle is the aliasing 2.0 exists to remove. Revisit only if setting it on a view you can't open DevTools on turns out to matter.
...
- **7.3 `SendRuntimeReady` had three call sites, not four — and world surfaces had none.** The three were `LoadSurface`, the view-recreate path and the host-restart loop. The interesting half: **world surfaces never greeted at all.** Created in their own loop with their own host process and renderer, they never pass through `LoadSurface` — 1.x marked them loaded by hand (`SetSurfaceLoaded(instance.viewId, true)`) precisely because of that — so nothing sent them `runtime.ready` and `osfui.ready` never resolved on a world surface. The page-initiated handshake fixes it for free: `MessageBridge::HandleHello` opens `_gates[viewId]` on demand, so a document that greets gets `ready`, its state replay and its event gate whether or not anything called `OnViewCreated` for it.
