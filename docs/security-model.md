...
5. **No arbitrary native bridge.** There is exactly one inbound message type (`ui.command`) with an explicit whitelist: surface control (`close`, `setVisible`, `menu.open` / `menu.close`, `hud.show` / `hud.hide`, `setViewHidden`), read/subscribe catalogs (`views.get`, `i18n.get`, `diagnostics.get`), diagnostics (`log`, `ping`), read-only game data (`game.get`), per-view input routing (`osfui.gamepadRaw`, `osfui.handleBack`), the fixed-target shell openers (`osfui.openModPage`, `osfui.openLogFolder`), the settings commands (`settings.get` / `settings.set` / `settings.reset` / `settings.captureKey`), owning-mod Papyrus delivery (`ui.action`, `ui.papyrusRequest`), and the platform-private bug reporter described below. Unknown types and commands are rejected and logged (warn-once per command name; the `ui.error` reply is sent every time). There is no "call function by name", no eval, no reflection. Enforced in `MessageBridge::HandleUiCommand`.
   ...
   - The **automatic bug reporter** is a narrow native egress exception, not network access for views. `diagnostics.reportStatus`, `diagnostics.submitReport`, and `osfui.openReportIssue` reject every caller except the exact built-in `osfui/settings` view. The endpoint comes only from host-owned `config.json`, must be a bounded HTTPS URL without whitespace, and redirects are disabled. Submission is impossible until the player opens the disclosure, supplies a title/description, and checks the consent box; the post-crash helper has a separate native Yes/No disclosure. Native collects at most 384 KiB from each of `OSF UI.log` and `OSF UI.webview2-host.log`, replaces known account/Documents/plugin roots before transmission, caps the whole JSON body at 1 MiB, and uses normal WinHTTP certificate verification. The service requires a renewable HMAC-signed installation ticket, applies fast per-location throttles plus Durable Object-backed daily limits (three per installation, five per source network and 100 total), and stores logs in a private R2 bucket. GitHub publication is asynchronous and single-concurrency through a retrying Queue; public prose has mentions and raw HTML neutralized, and never contains logs. Independent server flags pause intake or public issue creation without a client update. Reports are deleted after 30 days (daily cleanup plus an R2 lifecycle backstop); GitHub, administrator and signing keys exist only as Worker secrets. A declined prompt or disabled/failed endpoint sends nothing.
