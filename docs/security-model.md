...
5. **No arbitrary native bridge.** A view can send exactly two envelope kinds, and each one dispatches through an **explicit, name-keyed registry** — there is no generic "call native", no eval, no reflection, and no view message that supplies a function name. Enforced in `MessageBridge::HandleWebMessage` / `DispatchSend` / `DispatchRequest`.
   ...
   The registry is split by kind, and the split is itself a control. A `send` is a pure notification with nothing to settle; a `request` carries a caller-chosen id and settles exactly once. A `request` naming a strict send endpoint is rejected `wrong-endpoint-kind`, and a `send` naming a request endpoint is **dropped** — never quietly executed as if the caller had asked correctly, because running a mutation whose kind the caller got wrong is how the interesting bugs start. Native ABI `RegisterCommand` is the bounded compatibility exception described below. An `id` on a `send` is a hard `invalid-request` rather than a silent demotion, ids are bounded to 1–64 characters, and a payload that is present but not an object is refused rather than coerced.

   The platform surface is small enough to list. Sends (one-way): the handshake
   `osfui.hello` (answered by the bridge itself, not by a registered handler),
   surface control `close` / `setVisible`, `view.ready`, `log`, the per-view
   input-routing declarations `osfui.gamepadRaw` / `osfui.handleBack`, the
   platform-private `osfui.handoffRetry`, and owning-mod Papyrus delivery
   `papyrus.send`. Requests (settle payload-or-error): surface control
   ...
   platform-private startup-policy write `osfui.setViewAutoStart`, and the
   platform-private bug reporter (`diagnostics.reportStatus`,
   `diagnostics.submitReport`, `osfui.openReportIssue`).
   ...
   - A separate trusted native SFSE plugin can register additional endpoints via the exported `OSFUI_RequestBridge` API (`docs/native-plugin-api.md`): `RegisterCommand` claims a compatibility command that accepts `send` plus the ABI 1.x auto-ack request path, while `RegisterRequest` claims an explicitly settled request endpoint. Both names must be shaped `<author>.<modname>.<name>` — two dots minimum, with the leading mod id matching the public id grammar. Every platform endpoint is dotless or single-dot, so platform names are structurally unregisterable (this structural rule replaced an earlier reserved-prefix list, which could drift). Duplicate registrations are refused first-wins **across both registries**, so an already-claimed name cannot be hijacked or shadowed by re-registering it as the other kind. Enforced in `BridgeApi::RegisterCommand` / `RegisterRequest` (`IsValidPluginCommand`). This widens the surface only by what a mod author's own DLL deliberately exposes; untrusted JS still cannot register anything, and validating each added endpoint is that plugin's responsibility. ABI 1.8 accepts callers built against every 1.x minor; only a genuinely different major is refused.

   - `papyrus.send` and `papyrus.request` can reach only Papyrus listeners registered for the calling view's owning mod id; that destination is derived from the trusted source view id and never accepted from the payload. Arguments are scalar strings/numbers/booleans, and JavaScript receives no game object or arbitrary function name. Correlated requests have one first-wins listener per mod, bounded inflight capacity, host-minted one-shot reply tokens, a fixed timeout, and session teardown; only the owning mod's Papyrus code decides what game operation a request performs. `SetView*` and replies serialize form identities rather than exposing native objects.
...
7. **Clipboard follows Chromium's gesture rules.** There is no OSF UI bridge endpoint for clipboard access. In-page copy, cut, paste, and the async Clipboard API are handled by Chromium; OSF UI does not log or independently gate them. A user paste can disclose sensitive clipboard text to a view, and per-view clipboard gating remains future hardening.
...
8. **Outbound delivery is addressed, not subscribed.** 2.0 pushes state and events instead of answering `*.get` reads, so "what a view may *receive*" is now as much a part of the model as what it may call. Nothing a view sends can widen that set — there is no subscribe message.
   ...
   - **Mod state and mod events** (Papyrus `SetView*` / `SendViewEvent`, the ABI's `SetViewState` / `SendToWeb`) reach only the live views whose qualified id begins with the publishing mod's id, resolved fresh from the loaded surfaces on every publish. A view cannot receive another mod's state or events, and no message from a page changes its own address.
   - **`osfui/i18n`** is computed per view and carries only the catalog of that document's owning mod.
   - **`osfui/settings`, `osfui/views` and `osfui/diagnostics`** go to every live bridge-enabled view. `osfui/settings` is the whole registry — every installed mod's schema and values — so any bridged third-party view can read every mod's settings. This is an information-disclosure gap, not a write path, and 2.0 made it *broader* than 1.x's `settings.get`, because the document now arrives unasked at every greeting. It is listed under future hardening below and is the main reason to keep sensitive values out of settings.
...
- Versioned bridge API so views cannot probe for undocumented endpoints. Partially done: `ready` carries `bridgeVersion` (and the plugin `version`), and an unknown *send* is silent in release builds. The remaining gap is that an unknown *request* still answers `unknown-endpoint` in every build, which is enough to enumerate what exists.
