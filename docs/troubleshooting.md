...
**A mod is missing from Mod Settings:** check `OSF UI.log` for the settings filename and reason (bad filename, JSON parse error with line/column, corrupt saved values). A corrupt values file is renamed `<mod>.json.bad` and defaults are used; if you were hand-editing, fix the `.bad` file and rename it back.
...
| Crash opening the overlay with OptiScaler + Steam overlay | Fixed: current builds always composite through the Scaleform UI pass and never extend the wrapped Present chain. All three can stay enabled; confirm `UI-pass overlay armed`. |
