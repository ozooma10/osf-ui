...
Everything user-facing is in the in-game menu (F10 → OSF UI): open/close key, language, the pause-menu entry, game-key collision warnings, and under *Diagnostics* the render-stats readout, Debug mode (surfaces developer tools such as the Web Performance Lab), and **Bug reporting** — turn that off and neither the System Health reporter nor the post-crash prompt offers to send anything.
