...
- Native host-side unit tests live in `tests/native/` (`bash tests/native/run.sh`, needs a C++23
  compiler). With no clang/g++ on PATH — the case on this box — run.sh finds Visual Studio via
  vswhere and builds through MSVC cl itself; no dev shell or hand-written batch file needed. They
  compile real runtime sources against stubs; no game required.
